...
#define COAP_MID_CACHE_SIZE 3
    ...
    typedef struct coap_context_t
    {
        ...
        /*
         * whether coap_context as a proxy. 
         * if value eqal to 1, it is a proxy , othewise not.
         * default value is 0
         */
        unsigned int proxy_flag;

        /*
         * storage exchange informations about client and server
         */
        struct coap_proxy_map_t *proxy_map;
        
    } coap_context_t;

    /**
     * Set the coap context as a proxy
     * the flag eqal to 1, then it is a proxy, otherwise not
     */
     static inline void
     coap_set_proxy_flag(coap_context_t *context, unsigned int proxy)
     {
        context->proxy_flag = proxy;
     }

     /**
      * check if the coap context is a proxy
      */
      static inline unsigned int
      coap_is_proxy(coap_context_t *context)
      {
        return (((context->proxy_flag) == 1)? 1:0);
      }
